...
function seopress_google_analytics_optimize_callback() {
    $options = get_option('seopress_google_analytics_option_name');
    $check   = isset($options['seopress_google_analytics_optimize']) ? $options['seopress_google_analytics_optimize'] : null;
...
<p class="description">
    <?php _e('Google Optimize offers A/B testing, website testing & personalization tools.', 'wp-seopress'); ?>
...
function seopress_google_analytics_remarketing_callback() {
    $options = get_option('seopress_google_analytics_option_name');

    $check = isset($options['seopress_google_analytics_remarketing']); ?>

<label for="seopress_google_analytics_remarketing">
    <input id="seopress_google_analytics_remarketing"
        name="seopress_google_analytics_option_name[seopress_google_analytics_remarketing]" type="checkbox" <?php if ('1' == $check) { ?>
    checked="yes"
    <?php } ?>
    value="1"/>
    <?php _e('Enable remarketing, demographics, and interests reporting', 'wp-seopress'); ?>
</label>

<p class="description">
    <?php _e('A remarketing audience is a list of cookies or mobile-advertising IDs that represents a group of users you want to re-engage because of their likelihood to convert.', 'wp-seopress'); ?>
    <a class="seopress-help" href="https://support.google.com/analytics/answer/2611268?hl=en" target="_blank">
        <?php _e('Learn more', 'wp-seopress'); ?>
    </a>
    <span class="seopress-help dashicons dashicons-external"></span>
</p>

<?php if (isset($options['seopress_google_analytics_remarketing'])) {
        esc_attr($options['seopress_google_analytics_remarketing']);
    }
}

function seopress_google_analytics_ip_anonymization_callback() {
    $options = get_option('seopress_google_analytics_option_name');

    $check = isset($options['seopress_google_analytics_ip_anonymization']); ?>

<label for="seopress_google_analytics_ip_anonymization">
    <input id="seopress_google_analytics_ip_anonymization"
        name="seopress_google_analytics_option_name[seopress_google_analytics_ip_anonymization]" type="checkbox" <?php if ('1' == $check) { ?>
    checked="yes"
    <?php } ?>
    value="1"/>
    <?php _e('Enable IP Anonymization', 'wp-seopress'); ?>
</label>

<p class="description">
    <?php _e('When a customer of Analytics requests IP address anonymization, Analytics anonymizes the address as soon as technically feasible at the earliest possible stage of the collection network.', 'wp-seopress'); ?>
    <a class="seopress-help" href="https://developers.google.com/analytics/devguides/collection/gtagjs/ip-anonymization"
        target="_blank">
        <?php _e('Learn more', 'wp-seopress'); ?>
    </a>
    <span class="seopress-help dashicons dashicons-external"></span>
...
<?php if (isset($options['seopress_google_analytics_ip_anonymization'])) {
        esc_attr($options['seopress_google_analytics_ip_anonymization']);
    }
}

function seopress_google_analytics_link_attribution_callback() {
    $options = get_option('seopress_google_analytics_option_name');

    $check = isset($options['seopress_google_analytics_link_attribution']); ?>

<label for="seopress_google_analytics_link_attribution">
    <input id="seopress_google_analytics_link_attribution"
        name="seopress_google_analytics_option_name[seopress_google_analytics_link_attribution]" type="checkbox" <?php if ('1' == $check) { ?>
    checked="yes"
    <?php } ?>
    value="1"/>
    <?php _e('Enhanced Link Attribution', 'wp-seopress'); ?>
</label>

<p class="description">
    <?php _e('Enhanced Link Attribution improves the accuracy of your In-Page Analytics report by automatically differentiating between multiple links to the same URL on a single page by using link element IDs.', 'wp-seopress'); ?>
    <a class="seopress-help"
...
<?php if (isset($options['seopress_google_analytics_link_attribution'])) {
        esc_attr($options['seopress_google_analytics_link_attribution']);
    }
}

function seopress_google_analytics_cross_enable_callback() {
    $options = get_option('seopress_google_analytics_option_name');

    $check = isset($options['seopress_google_analytics_cross_enable']); ?>

<label for="seopress_google_analytics_cross_enable">
    <input id="seopress_google_analytics_cross_enable"
        name="seopress_google_analytics_option_name[seopress_google_analytics_cross_enable]" type="checkbox" <?php if ('1' == $check) { ?>
    checked="yes"
    <?php } ?>
    value="1"/>
    <?php _e('Enable cross-domain tracking', 'wp-seopress'); ?>
</label>

<p class="description">
    <?php _e('Cross domain tracking makes it possible for Analytics to see sessions on two related sites (such as an ecommerce site and a separate shopping cart site) as a single session. This is sometimes called site linking.', 'wp-seopress'); ?>
    <a class="seopress-help" href="https://developers.google.com/analytics/devguides/collection/gtagjs/cross-domain"
        target="_blank">
        <?php _e('Learn more', 'wp-seopress'); ?>
    </a>
    <span class="seopress-help dashicons dashicons-external"></span>
...
<?php if (isset($options['seopress_google_analytics_cross_enable'])) {
        esc_attr($options['seopress_google_analytics_cross_enable']);
    }
}

function seopress_google_analytics_cross_domain_callback() {
    $options = get_option('seopress_google_analytics_option_name');
    $check   = isset($options['seopress_google_analytics_cross_domain']) ? $options['seopress_google_analytics_cross_domain'] : null;
