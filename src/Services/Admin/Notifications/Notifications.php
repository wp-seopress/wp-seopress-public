...
class Notifications {
	/**
	 * The customOrder property.
	 *
	 * @var array
	 */
	private $custom_order = array( 'high', 'medium', 'low', 'info' );

	/**
	 * The do_migration property.
	 *
	 * @var bool
	 */
	public $do_migration = false;
	...
	public function __construct() {
		add_filter( 'seopress_dashboard_tasks', array( $this, 'filterTasks' ) );
	}

	/**
	 * The generateAllNotifications function.
	 *
	 * @return array
	 */
	public function generateAllNotifications() { // phpcs:ignore -- TODO: check if method is outside this class before renaming.
		...
		if ( false === get_theme_support( 'title-tag' ) && ( function_exists( 'wp_is_block_theme' ) && true !== wp_is_block_theme() ) ) {
	...
	/**
	 * Get severity notification.
	 *
	 * @param string $impact The impact.
	 *
	 * @return array
	 */
	public function getSeverityNotification( $impact = 'all' ) { // phpcs:ignore -- TODO: check if method is outside this class before renaming.
		$all = $this->generateAllNotifications();

		$sum             = 0;
		$severity_levels = array( 'high', 'medium', 'low', 'info' );

		foreach ( $severity_levels as $level ) {
			if ( 'all' === $impact || $impact === $level ) {
	 ...
	 */
	public function orderByImpact( $notifications ) { // phpcs:ignore -- TODO: check if method is outside this class before renaming.
		usort(
			$notifications,
			function ( $a, $b ) {
				$a_impact = isset( $a['impact'] ) ? array_key_first( $a['impact'] ) : ''; // Get the impact value if 'impact' key exists.
				$b_impact = isset( $b['impact'] ) ? array_key_first( $b['impact'] ) : ''; // Get the impact value if 'impact' key exists.

				$a_index = array_search( $a_impact, $this->custom_order, true );
				$b_index = array_search( $b_impact, $this->custom_order, true );
	 ...
	 */
	public function renderNotification( $args ) { // phpcs:ignore -- TODO: check if method is outside this class before renaming.
		if ( empty( $args ) ) {
			return;
		}
		...
		$impact     = isset( $args['impact'] ) ? $args['impact'] : array();
	 ...
	 * @return array tasks
	 */
	public function filterTasks( $tasks ) { // phpcs:ignore -- TODO: check if method is outside this class before renaming.
		$run_migration = $this->do_migration ? false : 'done';

		if ( false === $run_migration ) {
			$new_task =
			array(
				'done'  => $run_migration,
