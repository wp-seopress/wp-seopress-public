...
use SEOPress\Vendor\GuzzleHttp\Exception\ConnectException;
...
use SEOPress\Vendor\GuzzleHttp\Promise\FulfilledPromise;
use SEOPress\Vendor\GuzzleHttp\Promise\PromiseInterface;
use SEOPress\Vendor\GuzzleHttp\Psr7;
use SEOPress\Vendor\GuzzleHttp\Psr7\LazyOpenStream;
use SEOPress\Vendor\GuzzleHttp\Psr7\Uri;
use SEOPress\Vendor\GuzzleHttp\TransferStats;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    public function __construct(int $maxHandles, string $shareMode = TransportSharing::NONE, $shareHandle = null)
    {
        ...
        if ($shareHandle instanceof CurlShareHandleState) {
            ...
            $this->opaqueShareConnectionCache = \true;
        }
    ...
    public function create(RequestInterface $request, array $options): EasyHandle
    {
        ...
        if ('' === $protocolVersion) {
            \SEOPress\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Sending a request with an empty protocol version is deprecated; guzzlehttp/guzzle 8.0 will reject empty protocol versions.');
            $protocolVersion = '1.1';
            $request = Psr7\Utils::modifyRequest($request, ['version' => $protocolVersion]);
        }
        $multiplex = self::normalizeMultiplex($options);
        $requiredMultiplex = \in_array($multiplex, [Multiplexing::REQUIRE_EAGER, Multiplexing::REQUIRE_WAIT], \true);
    ...
    private static function normalizeMultiplex(array $options): ?string
    {
        ...
        if (!\in_array($multiplex, [Multiplexing::NONE, Multiplexing::EAGER, Multiplexing::WAIT, Multiplexing::REQUIRE_EAGER, Multiplexing::REQUIRE_WAIT], \true)) {
    ...
    private static function triggerConflictingCurlOptionDeprecations(array $options): void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            if ($replacement !== null) {
                \SEOPress\Vendor\trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed request handling. Use %s instead.', $name, $replacement));
                continue;
            }
            \SEOPress\Vendor\trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed cURL internals.', $name));
        }
    }
    private static function triggerUnsupportedCurlOptionDeprecations(array $options): void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            \SEOPress\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.12', \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject raw cURL options outside the built-in cURL handlers\' allow-list.', self::formatCurlOption($option)));
        }
    }
    private static function triggerUnsupportedRequestOptionDeprecations(array $options): void
    {
        if (\array_key_exists('stream_context', $options)) {
            \SEOPress\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "stream_context" request option to a cURL handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because cURL handlers ignore PHP stream context options.');
        }
    }
    /**
    ...
    private static function forceFreshConnectionForAuthenticatedProxy(RequestInterface $request, array &$conf): void
    {
        ...
        $conf[\CURLOPT_FRESH_CONNECT] = \true;
        $conf[\CURLOPT_FORBID_REUSE] = \true;
    }
    /**
     * @param array<int|string, mixed> $conf
     */
    ...
    private static function isSocksProxy(string $proxy, array $conf): bool
    {
        ...
        if ($scheme !== null) {
            if (\in_array($scheme, ['socks', 'socks4', 'socks4a', 'socks5', 'socks5h'], \true)) {
                return \true;
            }
    ...
    private function getDefaultConf(EasyHandle $easy): array
    {
        ...
        if ('2' === $version || '2.0' === $version) {
            ...
            if (\in_array($multiplex, [Multiplexing::WAIT, Multiplexing::REQUIRE_WAIT], \true) && CurlVersion::supportsMultiplex()) {
                ...
                $conf[(int) \constant('CURLOPT_PIPEWAIT')] = \true;
            }
        } elseif ('1.1' === $version) {
            if (\in_array($multiplex, [Multiplexing::REQUIRE_EAGER, Multiplexing::REQUIRE_WAIT], \true)) {
