                ...
                foreach ($mapping['children'] as $key => $child) {
                    ...
                    if ($maymatch) {
                        ...
                        if ($child['type'] != self::TYPE_CHOICE) {
                            ...
                            $maymatch = isset($temp['constant']) && $child['constant'] == $temp['constant'] && $temp['type'] == self::CLASS_CONTEXT_SPECIFIC;
                        }
                    }
                    if ($maymatch) {
                        // Attempt submapping.
                ...
                for ($i = 0; $i < count($decoded['content']); $i++) {
                    ...
                    foreach ($mapping['children'] as $key => $child) {
                        ...
                        if ($child['type'] != self::TYPE_CHOICE) {
                            ...
                            $maymatch = isset($temp['constant']) && $child['constant'] == $temp['constant'] && $tempClass == self::CLASS_CONTEXT_SPECIFIC;
                        }
                        if ($maymatch) {
                            // Attempt submapping.
                            $candidate = self::asn1map($temp, $child, $special);
